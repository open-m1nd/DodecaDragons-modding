...
*/
  ...
  // "rewritten" (mostly just changing variables) function because fire upgrade cost/bought is now an array
function buyFireUpgrade(x) {
  if (!game.fire.gte(game.fireUpgradeCost[x-1])) return; //this used to check if x exists, it no longer does that because honestly i feel like thats a bit retundant, just checks if you can afford the upgrade now
  if (game.fireUpgradeCost[x-1].lt("e1e9")) game.fire = game.fire.sub(game.fireUpgradeCost[x-1]); // honestly would love to know what this does (i have no clue and its never commented ever)
  game.fireUpgradeBought[x-1] = game.fireUpgradeBought[x-1].add(1);
  game.fireUpgradeCost[x-1] = new Decimal(fireUpgradeBase[x]).pow(game.fireUpgradeBought[x-1]).mul(fireUpgradeInitialCosts[x]).floor(); //update cost to reflect new count
  document.getElementById('fireUpgrade' + x + 'Cost').textContent = format(game.fireUpgradeCost[x-1], 0);
  // for whatever reason i made this an if statement instead of keeping it a switch, whatever doesnt matter that much i think...
  if (x==1) {
    //the effect of upgrade 1 depends on whether magic upgrade 13 has been purchased
    let upg1eff = game.magicUpgradesBought[13] ? new Decimal(3.5).pow(game.fireUpgradeBought[0].pow(0.7)) : new Decimal(2).pow(game.fireUpgradeBought[0].pow(0.6));
...
function fireBuyMax(x) {
  ...
  let amtToBuy = Decimal.affordGeometricSeries(amtToSpend, fireUpgradeInitialCosts[x], fireUpgradeBase[x], game.fireUpgradeBought[x-1]); //calculate how many we can afford
  let costToBuy = Decimal.sumGeometricSeries(amtToBuy, fireUpgradeInitialCosts[x], fireUpgradeBase[x], game.fireUpgradeBought[x-1]); //determine total cost
  if (costToBuy.lt("e1e9")) game.fire = Decimal.max(game.fire.sub(costToBuy), 0);
  game.fireUpgradeBought[x-1] = game.fireUpgradeBought[x-1].add(amtToBuy);
  game.fireUpgradeCost[x-1] = new Decimal(fireUpgradeBase[x]).pow(game.fireUpgradeBought[x-1]).mul(fireUpgradeInitialCosts[x]).floor(); //update cost to reflect new count
  document.getElementById('fireUpgrade' + x + 'Cost').textContent = format(game.fireUpgradeCost[x-1], 0); 
  switch (x) {
    case 1:
